...
import { cartesian3ToDegrees } from '../base'
import {
  gcj02towgs84,
  computeArea4Triangle,
  getTwoPointDistance,
  screenTo3D,
  positionToWindowPosition,
  makeLineSegment,
  verticesToLines,
  getSpaceDistance,
  getPolygonArea,
...
export default class PlotUtil {
  ...
  constructor(options) {
    ...
    this.floatingPointVisible = true // 绘制过程中移动的点显示隐藏（绘制交跨先，不添加浮动点，防止误触发）
    ...
    this.platform = 'pc'
    this.PickMapModel = options.PickMapModel
    this.MousePosition = options.MousePosition // 鼠标移动的回调方法
    ...
    this.firePoint = null // 着火点
    this.firePointStatus = false
    this.mouseTip = options.mouseTip // 是否显示右键提示
    this.positionInfo = options.positionInfo // 是否显示右上角坐标信息
    this.$MouseTip = null // 鼠标提示工具
    this.translucencyEnabled = options.translucencyEnabled // 开启球体半透明模式（兼容海外模型在地下问题）
  ...
  addKmlDataSource(url, cb) {
    const fakeViewer = new Cesium.Viewer(Vue.prototype.$vues.$refs.fakeContainer, {
      // 设置虚拟的 viewer 参数
      timeline: false,
      animation: false,
      shouldAnimate: false,
    ...
    this.KmlDataSource = fakeViewer.dataSources
      .add(
        Cesium.KmlDataSource.load(url, {
          camera: fakeViewer.scene.camera,
          canvas: fakeViewer.scene.canvas,
          clampToGround: true
        })
      )
      .then(kmlData => {
        if (cb) {
          if (
            ...
            const list = kmlData.entities.values
            // const list = kmlData.entities.values.filter(_ => _.polygon || _.polyline || _.point)
    ...
    //   )
    //   .then(kmlData => {
    //     console.log('........<entities', kmlData, kmlData.entities, kmlData.entities.values)
    //     if (cb) {
    //       if (kmlData.entities &&
    //         kmlData.entities.values &&
    //         kmlData.entities.values.length > 0) {
  ...
  handleShpData(feature, system, cb) {
    ...
    for (let index = 0; index < positions.length; index++) {
      ...
      if (system === 'GCJ-02') {
        const WGS84Position = gcj02towgs84(pos[0], pos[1])
    ...
    if (feature.geometry.type === 'Polygon') {
      drawingMode = 'polygon'
      _centerPoint = cartesian3ToDegrees(Cesium.BoundingSphere.fromPoints(Cartesian3Position).center)
  ...
  async handleKmlData(entity, system, cb) {
    ...
    if (entity.polygon) {
      ...
      drawingMode = 'point'
      Cartesian3Position = [entity.position.getValue(viewer.clock.currentTime)]
    }
    if (!drawingMode) {
      cb()
      return false
    }
    const positions = Cartesian3Position.map(_ => cartesian3ToDegrees(_))
    let _positions = positions
    if (system === 'GCJ-02') {
      _positions = positions.map((_) => {
        const WGS84Position = gcj02towgs84(_.longitude, _.latitude)
        return {
          longitude: WGS84Position[0],
          latitude: WGS84Position[1],
        }
    ...
    for (let index = 0; index < _positions.length; index++) {
      ...
      // ^ 计算 KML 数据顶点在高程上的海拔
      const tPoint = await getTerrainPosition({
        longitude: point.longitude,
        latitude: point.latitude,
        altitude: 0
      })
      point.altitude = tPoint.altitude || 0
    ...
    if (drawingMode === 'polygon') {
      _activeShapeComputed = getPolygonArea(areaPoints)
      _centerPoint = cartesian3ToDegrees(Cesium.BoundingSphere.fromPoints(_Cartesian3Position).center)
  ...
  returnPosition(positions, index) {
    ...
    return {
      _heightPos: cartesian3ToDegrees(Cesium.Cartesian3.fromRadians(cartographic.longitude, cartographic.latitude, height)),
  ...
  // 移除着火点
  removeFirePoint() {
    viewer.scene.primitives.remove(this.firePoint)
  }

  // 添加着火点图形
  addFirePoint() {
    const entity = this.vertexEntities[0] || this.floatingPoint
    if (!entity) return
    this.firePoint = new Cesium.ParticleSystem({
      image: './smoke.png',
    ...
    viewer.scene.primitives.add(this.firePoint)
  }

  // 设置着火点状态
  SetFirePointStatus(status) {
    this.firePointStatus = status
  }

  // 设置着火点图形
  SetFirePointVisible(status) {
    if (this.drawingMode === 'point') {
      this.removeFirePoint()
      if (status) {
        this.addFirePoint()
      }
      const entity = this.vertexEntities[0] || this.floatingPoint
      if (entity) {
        entity.point.show = !status
      }
    }
  }
  ...
  AddEditPlotData(data) {
    ...
    if (data.objectInfo.towerSns) {
      _towerSns = data.objectInfo.towerSns
    }
    ...
    if (this.drawingMode === 'polygon') {
      ...
      if (this.activeSubLine.length > 0) {
        ...
        this.setTotalDistancePositionAndLabel(this.activeShapePoints[this.activeShapePoints.length - 1], this.activeShapeComputed)
      }
    }
    this.terminateShape()
  }
  ...
  ContinueDrawing(position) {
    if (position) {
      ...
      const windowPosition = positionToWindowPosition(position)
  ...
  addMouseTip() {
    ...
    this.$MouseTip = new MouseTip({
      mouseTip: this.mouseTip,
      positionInfo: this.positionInfo
    })
  }
  setMouseTipPosition(mousePosition) {
    if (this.$MouseTip) {
      this.$MouseTip.UpdatePosition(mousePosition)
    }
  }
  ...
  setVerticesPositionValue() {
    if (this.VerticesFinish) {
      const _verticesPosition = this.verticesPosition.filter(_ => _).map(_ => cartesian3ToDegrees(_))
  ...
  getMouseCurrentPosition(Cartesian3) {
    ...
    if (window.Cesium.defined(canvasPosition)) {
      const mousePosition = {
        map: cartesian3ToDegrees(Cartesian3),
        screen: canvasPosition
      }
      this.setMouseTipPosition(mousePosition)
  ...
  terminateShape() {
    ...
    const _centerPoint = cartesian3ToDegrees(centerPoint)
    const _activeShapePoints = this.activeShapePoints.filter(_ => _).map(_ => cartesian3ToDegrees(_))
    const _verticesPosition = this.verticesPosition.filter(_ => _).map(_ => cartesian3ToDegrees(_))
    if (this.Finish) {
      this.Finish({
        ...
        firePoint: this.firePointStatus,
        towerSns: this.towerSns
      })
      // 着火点
      this.SetFirePointVisible(this.firePointStatus)
    }
  ...
  createPoint(earthPosition, id) {
    ...
    if (this.drawingMode === 'text') {
      ...
      if (this.pointPositionVisible) {
        const _position = cartesian3ToDegrees(earthPosition)
  ...
  drawLineLabel() {
    if (this.activeShapePoints && this.activeShapePoints.length > 0) {
      const _activeShapePoints = this.activeShapePoints.map(_ => cartesian3ToDegrees(_))
  ...
  createAreaLabel(centerPoint, area) {
    // 用海拔最高的顶点做中心点的高
    const altitudes = this.verticesPosition.filter(_ => _).map(_ => cartesian3ToDegrees(_)).map(_ => _.altitude)
    const max = altitudes.sort()[altitudes.length - 1]
    const position = cartesian3ToDegrees(centerPoint)
  ...
  drawAreaLabel() {
    if (this.activeShapePoints && this.activeShapePoints.length > 1) {
      ...
      const areaPoints = []
      const _verticesPosition = this.activeShapePoints.map(_ => cartesian3ToDegrees(_))
      const firstVertex = _verticesPosition[0]
      _verticesPosition.push(firstVertex)
    ...
    if (this.plotting) {
      if (this.drawingMode === 'point' || this.drawingMode === 'text') {
        if (this.verticesPosition.length < 1) {
          flag = true
          message = Vue.prototype.$language('leastOnePointTip')
        }
      } else if (this.drawingMode === 'polyline') {
        if (this.verticesPosition.length < 2) {
          flag = true
          message = Vue.prototype.$language('leastTwoPointTip')
        }
      } else if (this.drawingMode === 'polygon') {
        if (this.verticesPosition.length < 3) {
          flag = true
          message = Vue.prototype.$language('leastThreePointTip')
        }
      }
    }
    if (flag && message) {
  ...
  mouseMoveAction(movement, position) {
    ...
    if (position && position instanceof Cesium.Cartesian3) {
      ...
      try {
        newPosition = screenTo3D(movement.endPosition).cartesian3
      } catch (error) {
        console.log(error)
      }
    }
    ...
    if (this.plotting) {
      ...
      if (this.overlookStatus) {
        ...
        const _position = cartesian3ToDegrees(cartesian)
        ...
        const cartographic = cartesian3ToDegrees(this.pickedEntity.position.getValue(viewer.clock.currentTime))
      ...
      if (this.pointPositionVisible) {
        const text = formatGraphicLabelText('point', cartesian3ToDegrees(position2))
  ...
  leftClickAction(event, position) {
    ...
    if (this.plotting) {
      const mouseResult = screenTo3D(event.position)
      let earthPosition = null
      let floating = false
      if (this.platform === 'pc' && this.floatingPointVisible) {
        floating = true
      }
      if (position && position instanceof Cesium.Cartesian3) {
        ...
        earthPosition = mouseResult.Cartesian3
        if (mouseResult.pickModel) {
          if (mouseResult.pickModel.id) {
          ...
          } else if (mouseResult.pickModel.primitive) {
            const id = mouseResult.pickModel.primitive.id
            if (id && id.indexOf('mapMpdel' > -1)) {
              if (this.PickMapModel) {
                this.PickMapModel(parseInt(id.split('_')[1]))
              }
            }
          }
        }
      ...
      if (Cesium.defined(earthPosition)) {
        if (this.activeShapePoints.length === 0) {
          if (floating) {
            ...
            if (mouseResult.pickModel && mouseResult.pickModel.id && mouseResult.pickModel.id.id) {
              this.towerSns.push(mouseResult.pickModel.id.id)
            }
      ...
      if (pickModel && pickModel.id && pickModel.id instanceof Cesium.Entity) {
        ...
        if (_id.indexOf('_vertices') > -1) {
          ...
          const index = parseInt(this.editEntityId.split('_')[2])
          MessageBox.confirm(`是否删除顶点${index + 1}`, '', {
